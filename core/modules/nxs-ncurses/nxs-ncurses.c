...
static int						nxs_ncurses_mkwin					(nxs_ncurses_ctx_t *ctx, u_int index_win, u_int nlines, u_int ncols, u_int y, u_int x, nxs_ncurses_type_t type, nxs_ncurses_win_conf_t w_conf);
...
void nxs_ncurses_init(nxs_process_t *proc, nxs_ncurses_ctx_t *ctx, u_char *locale, nxs_bool_t key_mode, nxs_bool_t keypad_mode, void (*sig_resize)(int, void *))
{
	nxs_ncurses_win_t	*w;
 ...
 * Данная схема будет переопределена либо при повторном использовании данной функции (с другим color_pair), либо при активации/деактивации окна
 */
...
static int nxs_ncurses_mkwin(nxs_ncurses_ctx_t *ctx, u_int index_win, u_int nlines, u_int ncols, u_int y, u_int x, nxs_ncurses_type_t type, nxs_ncurses_win_conf_t w_conf)
{
	...
	if(nw->visible == NXS_YES){
		...
		if(nw->parent_index == NXS_NCURSES_STDWIN_PARENT ){
			...
			/*
			 * С использованием derwin() не удалось добиться корректной перерисовки родительского окна, удалённое окно продолжало отображаться.
			 * Помогал только wclear(), но затраты на проработку модуля таким методом были слишком большими.
			 *
			 * upd: если отказаться от derwin() и перейти к использованию только newwin (а иерархию организовать в данном модуле), то
