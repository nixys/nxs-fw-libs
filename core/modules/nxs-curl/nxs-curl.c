...
int nxs_curl_query(nxs_process_t *proc, nxs_curl_t *curl, nxs_rest_api_common_cmd_type_t method, u_char *query_str, ...)
{
	...
	if(res != CURLE_OK){
		...
		if(curl->log_type & NXS_CURL_LOG_ERROR){
			...
			nxs_log_write_debug(proc, "failed to process curl query get: %s (uri: \"%s\")", curl_easy_strerror(res), nxs_string_str(&query));
		}
	...
	if(curl->log_type & NXS_CURL_LOG_DEBUG){
		...
		nxs_log_write_debug(proc, "got data from server (response code: %zu, request url: \"%s\", response body: \"%s\")", curl->ret_code, nxs_string_str(&query), nxs_buf_get_subbuf(&curl->out_buf, 0));
	}
