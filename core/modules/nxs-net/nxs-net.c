...
static int					nxs_net_send_file_metadata_prep				(nxs_process_t *proc, nxs_net_connect_t *con, nxs_string_t *path, nxs_metadata_v_t fill_meta, nxs_metadata_t *metadata, off_t *f_size);
//static void					nxs_net_send_file_metadata_prep_empty		(nxs_net_connect_t *con);

static int					nxs_net_metadata_recv						(nxs_process_t *proc, nxs_net_connect_t *con, time_t timeout, size_t offset, nxs_metadata_t *md, size_t *msize);
...
int nxs_net_connect(nxs_process_t *proc, nxs_net_connect_t *con)
{
	...
	for(i = 0; i < con->con_retr; i++){
		...
		for(p = r; p != NULL; p = p->ai_next){
			...
			nxs_log_write_debug_net(proc, "connect error: %s (ip: %s:%s)", strerror(errno), inet_ntoa(ip->sin_addr), nxs_string_str(&con->peer_port));
...
next_iteration:
			...
			nxs_log_write_debug_net(proc, "connect attempt failed (ip: %s:%s)", inet_ntoa(ip->sin_addr), nxs_string_str(&con->peer_port));
...
int nxs_net_unix_server_init(nxs_process_t *proc, nxs_net_unix_server_t *srv, u_char *sock_path, int con_queue_size, u_char *user, u_char *group, mode_t *mode)
{
	...
	if(strlen((char *)sock_path) > NXS_NET_UNIX_PATH_MAX - 1){
		...
		nxs_log_write_debug_net(proc, "server socket init error: socket path is too long (len: %d, max len: %d, sock path: \"%s\")", strlen((char *)sock_path), NXS_NET_UNIX_PATH_MAX - 1, sock_path);
	...
	if(user != NULL){
		...
		if((usr = getpwnam((char *)user)) == NULL){
			...
			nxs_log_write_debug_net(proc, "server socket init error: can't resolve user name (sock path: \"%s\", user name: \"%s\")", sock_path, user);
	...
	if(group != NULL){
		...
		if((grp = getgrnam((char *)group)) == NULL){
			...
			nxs_log_write_debug_net(proc, "server socket init error: can't resolve group name (sock path: \"%s\", group name: \"%s\")", sock_path, group);
...
int nxs_net_unix_connect_out_init(nxs_process_t *proc, nxs_net_connect_t *con, u_char *sock_path, int con_retr, u_char *header)
{
	...
	if(strlen((char *)sock_path) > NXS_NET_UNIX_PATH_MAX - 1){
		...
		nxs_log_write_debug_net(proc, "connection init error: socket path is too long (len: %d, max len: %d, sock path: \"%s\")", strlen((char *)sock_path), NXS_NET_UNIX_PATH_MAX - 1, sock_path);
...
int nxs_net_unix_server_close(nxs_process_t *proc, nxs_net_unix_server_t *srv)
{
	...
	if(srv->l_sock == NXS_NET_INIT_SOCK){
		...
		if(close(srv->l_sock) == -1){
			...
			nxs_log_write_debug_net(proc, "server socket close error: %s (sock fd: %d, sock path: \"%s\")", strerror(errno), srv->l_sock, nxs_string_str(&srv->l_sock_path));
...
int nxs_net_unix_server_unlink(nxs_process_t *proc, nxs_net_unix_server_t *srv)
{
	...
	if(srv->l_sock == NXS_NET_INIT_SOCK){
		...
		if(nxs_fs_unlink(&srv->l_sock_path) < 0){
			...
			nxs_log_write_debug_net(proc, "server socket unlink error: %s (sock path: \"%s\")", strerror(errno), nxs_string_str(&srv->l_sock_path));
...
int nxs_net_unix_connect_close(nxs_process_t *proc, nxs_net_connect_t *con)
{
	...
	if(con->sock == NXS_NET_INIT_SOCK){
		...
		if(close(con->sock) == -1){
			...
			nxs_log_write_debug_net(proc, "socket close error: %s (sock fd: %d, sock path: \"%s\")", strerror(errno), con->sock, nxs_string_str(&con->sock_path));
...
int nxs_net_unix_listen(nxs_process_t *proc, nxs_net_unix_server_t *srv)
{
	...
	if((srv->l_sock = socket(AF_UNIX, SOCK_STREAM, 0)) == -1){
		...
		nxs_log_write_debug_net(proc, "unix socket create error: %s (sock path: \"%s\")", strerror(errno), nxs_string_str(&srv->l_sock_path));
	...
	if(chown((char *)nxs_string_str(&srv->l_sock_path), srv->uid, srv->gid) < 0){
		...
		nxs_log_write_debug_net(proc, "unix socket chown error: %s (sock path: \"%s\", uid: %d, gid: %d)", strerror(errno), nxs_string_str(&srv->l_sock_path), srv->uid, srv->gid);
	...
	if(chmod((char *)nxs_string_str(&srv->l_sock_path), srv->mode) < 0){
		...
		nxs_log_write_debug_net(proc, "unix socket chmod error: %s (sock path: \"%s\", mode: %d)", strerror(errno), nxs_string_str(&srv->l_sock_path), srv->mode);
	...
	if(listen(srv->l_sock, srv->l_con_queue) == -1){
		...
		nxs_log_write_debug_net(proc, "unix socket listen error: %s (sock path: \"%s\")", strerror(errno), nxs_string_str(&srv->l_sock_path));
...
int nxs_net_unix_connect(nxs_process_t *proc, nxs_net_connect_t *con)
{
	...
	if((con->sock = socket(AF_UNIX, SOCK_STREAM, 0)) == -1){
		...
		nxs_log_write_debug_net(proc, "socket create error: %s (sock path: \"%s\")", strerror(errno), nxs_string_str(&con->sock_path));
		...
		if(nb > 0){
			...
			if(nb < 0){
				...
				if(con->type == NXS_NET_SOCK_TYPE_INET){
					...
					nxs_log_write_debug_net(proc, "recv error: %s (sock: %d, client: %s:%s, poll: %d)", strerror(errno), con->sock, nxs_string_str(&con->peer_ip), nxs_string_str(&con->peer_port), nxs_poll_get_res(con->poll_recv, con->sock));
				}
				else{

					nxs_log_write_debug_net(proc, "recv error: %s (sock: %d, sock path: \"%s\", poll: %d)", strerror(errno), con->sock, nxs_string_str(&con->sock_path), nxs_poll_get_res(con->poll_recv, con->sock));
				}
			...
			if(con->type == NXS_NET_SOCK_TYPE_INET){
				...
				nxs_log_write_debug_net(proc, "recv error: peer closed connection (sock: %d, client: %s:%s, poll: %d)", con->sock, nxs_string_str(&con->peer_ip), nxs_string_str(&con->peer_port), nxs_poll_get_res(con->poll_recv, con->sock));
			}
			else{

				nxs_log_write_debug_net(proc, "recv error: peer closed connection (sock: %d, sock path: \"%s\", poll: %d)", con->sock, nxs_string_str(&con->sock_path), nxs_poll_get_res(con->poll_recv, con->sock));
			}
		...
		if(nxs_poll_check_events(con->poll_send, con->sock, POLLIN)){
			...
			if((nb = recv(con->sock, t_buf, 1, MSG_PEEK)) == 0){
				...
				if(con->type == NXS_NET_SOCK_TYPE_INET){
					...
					nxs_log_write_debug_net(proc, "send error: peer closed connection (sock: %d, client: %s:%s, poll: %d)", con->sock, nxs_string_str(&con->peer_ip), nxs_string_str(&con->peer_port), nxs_poll_get_res(con->poll_send, con->sock));
				}
				else{

					nxs_log_write_debug_net(proc, "send error: peer closed connection (sock: %d, sock path: \"%s\", poll: %d)", con->sock, nxs_string_str(&con->sock_path), nxs_poll_get_res(con->poll_send, con->sock));
				}
				...
				if(nb < 0){
					...
					if(con->type == NXS_NET_SOCK_TYPE_INET){
						...
						nxs_log_write_debug_net(proc, "send error: %s (sock: %d, client: %s:%s, poll: %d)", strerror(errno), con->sock, nxs_string_str(&con->peer_ip), nxs_string_str(&con->peer_port), nxs_poll_get_res(con->poll_send, con->sock));
					}
					else{

						nxs_log_write_debug_net(proc, "send error: %s (sock: %d, sock path: \"%s\", poll: %d)", strerror(errno), con->sock, nxs_string_str(&con->sock_path), nxs_poll_get_res(con->poll_send, con->sock));
					}
...
ssize_t nxs_net_send(nxs_process_t *proc, nxs_net_connect_t *con, time_t timeout, void *data, int type)
{
	...
	switch(type){
		...
		case NXS_NET_DATA_TYPE_LIST:
			...
			for(i = 0, flag = NXS_NET_FLAG_BUF, buf = nxs_list_ptr_init(NXS_LIST_PTR_INIT_HEAD, lst); buf != NULL; buf = nxs_list_ptr_next(lst), i++){
 ...
 * 					Если metadata не NULL - она НЕ ДОЛЖНА содержать никаких данных, иначе это приведёт к утечке памяти, т.к.
...
int nxs_net_recv_file(nxs_process_t *proc, nxs_net_connect_t *con, nxs_net_opt_file_recv_t *opt, nxs_metadata_t *metadata)
{
	...
	if((ofd = nxs_fs_open(&path, O_WRONLY | O_CREAT | O_TRUNC, _NXS_NET_DEFAULT_PERM)) < 0){
		...
		nxs_log_write_debug_net(proc, "file recv error: can't open dst file: %s (file: \"%s\")", strerror(errno), nxs_string_str(&path));
	...
	while(lb > 0){
		...
		if((wb = write(ofd, rcv_buf, (size_t)eb)) < 0 || wb != (ssize_t)eb){
			...
			if(wb < 0){
				...
				nxs_log_write_debug_net(proc, "file recv error: can't write into dst file: %s (file: \"%s\")", strerror(errno), nxs_string_str(&path));
			}
			else{

				nxs_log_write_debug_net(proc, "file recv error: can't write into dst file: wrote less bytes then expected (file: \"%s\")", nxs_string_str(&path));
			}
	...
	if(opt->allow_attrs == NXS_YES){
		...
		if(nxs_metadata_get_uid(pmd, &uid) == NXS_METADATA_E_OK){
			...
			if(nxs_metadata_get_gid(pmd, &gid) == NXS_METADATA_E_OK){
				...
				if(chown((char *)nxs_string_str(&path), uid, gid) < 0){
					...
					nxs_log_write_debug_net(proc, "file recv error: chown error: %s (file: \"%s\")", strerror(errno), nxs_string_str(&path));
		...
		if(nxs_metadata_get_perm(pmd, &perm) == NXS_METADATA_E_OK){
			...
			if(chmod((char *)nxs_string_str(&path), perm) < 0){
				...
				nxs_log_write_debug_net(proc, "file recv error: chmod error: %s (file: \"%s\")", strerror(errno), nxs_string_str(&path));
		...
		if(nxs_metadata_get_atime(pmd, &ftime.actime) == NXS_METADATA_E_OK){
			...
			if(nxs_metadata_get_mtime(pmd, &ftime.modtime) == NXS_METADATA_E_OK){
				...
				if(utime((char *)nxs_string_str(&path), &ftime) < 0){
					...
					nxs_log_write_debug_net(proc, "file recv error: utime error: %s (file: \"%s\")", strerror(errno), nxs_string_str(&path));
 ...
 *
 * 					Если указатель не NULL и при этом поле "opt->fill_meta"	имеет значение NXS_METADATA_V_NONE - в переданной структуре
 * 					должны содержаться данные отправляемого файла (на случай, если необходимо строго задать имя отправляемого файла,
 * 					права доступа и др;	важно: размер файла (в структуре "metadata" в любом случае будет переопределён на реальный размер файла)).
 *
 * 					Если указатель не NULL и при этом поле "opt->fill_meta" имеет значение отличное NXS_METADATA_V_NONE (т.е. содержит номер конкретной
 * 					версии) - структура метаданных "metadata" будет очищена и заполнена информацией в соответствии с требуемой версией метаданных
 * 					для конкретного файла. Это может постребоваться для последующей обработки информации по отправленному файлу после завершения данной
 * 					функции.
 *
 * 					Если "metadata" равна NULL - будет использована временна структура метаданных, которая очищается после выполнения функции.
 * 					При этом, если "opt->fill_meta" имеет значение отличное NXS_METADATA_V_NONE (т.е. содержит номер конкретной версии) -
 * 					будут использованы метаданные указанной версии, иначе будут использованы метаданные версии NXS_METADATA_V2
 ...
 *
 * Критические ошибки (рекомендуется разрыв соединения, т.к. в канале могут остаться данные, которые удалённая сторона не сможет обрабоатать):
 * * NXS_NET_E_POLL				- ошибка poll
 * * NXS_NET_E_TIMEOUT			- тайм-аут при отправке данных
 * * NXS_NET_E_PEER_SHUTDOWN	- удалённая сторона закрыла соединение
 * * NXS_NET_E_SOCKWRITE		- ошибка записи в сокет
...
int nxs_net_send_file(nxs_process_t *proc, nxs_net_connect_t *con, nxs_net_opt_file_send_t *opt, nxs_string_t *path, nxs_metadata_t *metadata)
{
	...
	if((i_rc = nxs_fs_file_check_upd_create(proc, &fc, path)) != NXS_FS_E_OK){
		...
		nxs_log_write_debug_net(proc, "file send error: can't watch for sending file (fs ret code: %d, path: \"%s\")", i_rc, nxs_string_str(path));
	...
	if(opt->confirm == NXS_YES){
		...
		if(rc == NXS_NO){
			...
			nxs_log_write_debug_net(proc, "file send: confirm from peer is negative (confirm text: \"%s\", sending file: \"%s\")", nxs_buf_get_subbuf(&con->tmp_buf, 0), nxs_string_str(path));

			ec = NXS_NET_E_CONFIRM_NEGATIVE;
		}
		else{

			nxs_log_write_debug_net(proc, "file send: confirm from peer is positive (confirm text: \"%s\", sending file: \"%s\")", nxs_buf_get_subbuf(&con->tmp_buf, 0), nxs_string_str(path));
		}
...
/*
 * Функция для проверки правильности заполнения всех нужных полней в структуре opt.
 * На текущий момент эта функция всегда возвращает NXS_TRUE, т.к. пока список опций такой, что проверки не требуется. Однако, для того, чтобы
...
static size_t nxs_net_header_set_metadata(nxs_net_connect_t *con, nxs_metadata_t *md)
{
	off_t	dsize; // тип off_t используется для того, чтобы в функции nxs_buf_cpy_dyn() мы записали нужную информацию длиной sizeof(off_t) байт
...
static ssize_t nxs_net_send_buf(nxs_process_t *proc, nxs_net_connect_t *con, time_t timeout, nxs_buf_t *data, nxs_net_hdr_f_t flag)
{
	 ...
	 * Если con->gost_el == NULL, т.е. если шифрование отключено - данные шифроваться не будут (проверка производится в nxs_crypt_gost_buf())
	 */
	nxs_crypt_gost_buf(&con->tmp_buf, con->gost_el);
 ...
 * Если данная функция вернула код отличный от NXS_NET_E_OK - необходимо отправить заголовок (файловый) с указанным флагом NXS_NET_FLAG_FILE_CANCEL
 * это отменит дальнейшую передачу файла и позволит сохранить соединение (за счёт того, что в него не будет записана лишняя информация, которую
...
static int nxs_net_send_file_metadata_prep(nxs_process_t *proc, nxs_net_connect_t *con, nxs_string_t *path, nxs_metadata_v_t fill_meta, nxs_metadata_t *metadata, off_t *f_size)
{
	...
	if((rc = nxs_fs_get_file_type(path, &type)) != 0){
		...
		nxs_log_write_debug_net(proc, "file send error: can't stat for file: %s (path: \"%s\")", strerror(errno), nxs_string_str(path));
	...
	if(vmd != NXS_METADATA_V_NONE){
		...
		if((rc = nxs_metadata_fill(pmd, vmd, path)) != NXS_METADATA_E_OK){
			...
			nxs_log_write_debug_net(proc, "file send error: can't fill metadata (error code: %d, path: \"%s\")", rc, nxs_string_str(path));
...
error:
	...
	if(ec == NXS_NET_E_OK){
		...
		/*
		 * Расчёт размера всех данных, которые будут переданы удалённой стороне за исключением основного заголовка (размер файла + вся служебная информация)
		 *
		 * msize - размер метаданных
		 * f_size - размер файла
		 * f_size / _NXS_NET_SENDFILE_BUF_SIZE + 1 - количество управляющих байт для текущего размера файла (1 байт на каждые 64К данных)
 ...
 * Структура будет очищена и инициализирована заново (с нужной версией) в том случае, если память непосредственно под метаданные либо не выделена вовсе, либо
