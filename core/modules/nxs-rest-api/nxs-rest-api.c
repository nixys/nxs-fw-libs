...
typedef struct			nxs_rest_api_keyval_s			nxs_rest_api_keyval_t;
typedef struct			nxs_rest_api_handler_s			nxs_rest_api_handler_t;

/* Module declarations */

struct nxs_rest_api_keyval_s
{
	nxs_string_t				key;
	nxs_string_t				value;
};

struct nxs_rest_api_handler_s
{
...
nxs_rest_api_err_t nxs_rest_api_init(nxs_process_t *           proc,
                                     ...
                                     nxs_string_t *            crt_chain,
                                     nxs_string_t *            crt_pkey)
{
	...
	if((ctx->ev_handle = evhttp_bind_socket_with_handle(ctx->ev_httpd, (char *)nxs_string_str(&ctx->listen_ip), ctx->listen_port)) ==
	   NULL) {
...
static void nxs_rest_api_generic_handler(struct evhttp_request *_req, void *arg)
{
	...
	switch(evhttp_request_get_command(_req)) {
		...
		case EVHTTP_REQ_GET:
			req.cmd_type = NXS_REST_API_COMMON_CMD_GET;
			break;
		case EVHTTP_REQ_POST:
			req.cmd_type = NXS_REST_API_COMMON_CMD_POST;
			break;
		case EVHTTP_REQ_HEAD:
			req.cmd_type = NXS_REST_API_COMMON_CMD_HEAD;
			break;
		case EVHTTP_REQ_PUT:
			req.cmd_type = NXS_REST_API_COMMON_CMD_PUT;
			break;
		case EVHTTP_REQ_DELETE:
			req.cmd_type = NXS_REST_API_COMMON_CMD_DELETE;
			break;
		case EVHTTP_REQ_OPTIONS:
			req.cmd_type = NXS_REST_API_COMMON_CMD_OPTIONS;
			break;
		case EVHTTP_REQ_TRACE:
			req.cmd_type = NXS_REST_API_COMMON_CMD_TRACE;
			break;
		case EVHTTP_REQ_CONNECT:
			req.cmd_type = NXS_REST_API_COMMON_CMD_CONNECT;
			break;
		case EVHTTP_REQ_PATCH:
			req.cmd_type = NXS_REST_API_COMMON_CMD_PATCH;
			break;
...
error:
	...
	for(i = 0; i < nxs_array_count(&req.uri_out_headers); i++) {
		...
		evhttp_add_header(
		        evhttp_request_get_output_headers(_req), (char *)nxs_string_str(&kv->key), (char *)nxs_string_str(&kv->value));
	}
