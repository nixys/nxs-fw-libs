 ...
 * Сомнения в следующем: в качестве входных аргументов передаются указатели на элементы pipesem, которые являются двумя частями одного целого
...
int nxs_pipesem_write(nxs_process_t *proc, nxs_pipesem_t *out, size_t size, void *data)
{
	...
	if(*out->lock != NXS_PIPESEM_LOCK_OPEN){
		 ...
		 * Т.е. если канал закрыт для записи - проверяется активен ли он вообще (т.к. может получиться так, что парный процесс "закрыл" замок
		 * и завершился).
		 */
