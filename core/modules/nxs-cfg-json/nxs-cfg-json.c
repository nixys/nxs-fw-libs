...
static nxs_cfg_json_state_t				nxs_cfg_json_type_handler_array_object					(nxs_process_t *proc, nxs_json_t *json, nxs_cfg_json_par_t *cfg_json_par_el);
static nxs_cfg_json_state_t				nxs_cfg_json_type_handler_array_int						(nxs_process_t *proc, nxs_json_t *json, nxs_bool_t is_list, nxs_cfg_json_par_t *cfg_json_par_el);
static nxs_cfg_json_state_t				nxs_cfg_json_type_handler_array_int_16					(nxs_process_t *proc, nxs_json_t *json, nxs_bool_t is_list, nxs_cfg_json_par_t *cfg_json_par_el);
static nxs_cfg_json_state_t				nxs_cfg_json_type_handler_array_int_32					(nxs_process_t *proc, nxs_json_t *json, nxs_bool_t is_list, nxs_cfg_json_par_t *cfg_json_par_el);
static nxs_cfg_json_state_t				nxs_cfg_json_type_handler_array_real					(nxs_process_t *proc, nxs_json_t *json, nxs_bool_t is_list, nxs_cfg_json_par_t *cfg_json_par_el);
static nxs_cfg_json_state_t				nxs_cfg_json_type_handler_array_string					(nxs_process_t *proc, nxs_json_t *json, nxs_bool_t is_list, nxs_cfg_json_par_t *cfg_json_par_el);
static nxs_cfg_json_state_t				nxs_cfg_json_type_handler_array_bool					(nxs_process_t *proc, nxs_json_t *json, nxs_bool_t is_list, nxs_cfg_json_par_t *cfg_json_par_el);
...
nxs_cfg_json_state_t nxs_cfg_json_element_process(nxs_process_t *proc, nxs_json_t *json, nxs_cfg_json_par_t *cfg_json_par)
{
	...
	if(nxs_json_type_get(json) != NXS_JSON_TYPE_ARRAY && nxs_json_type_get(json) != NXS_JSON_TYPE_OBJECT){
		...
		nxs_log_write_error(proc, "json cfg process error: json object has incorrect type (key: \"%s\")", nxs_string_str(nxs_json_get_key(json)));
	...
	for(i = 0; i < nxs_json_child_get_count(json); i++){
		...
		if((p = nxs_cfg_json_find_par(proc, j, cfg_json_par)) == NULL){
			...
			nxs_log_write_error(proc, "json cfg process error: unknown option (option: \"%s\")", nxs_string_str(nxs_json_get_key(j)));

			rc = NXS_CFG_JSON_CONF_ERROR;
			goto error;
		}

		/* Добавление найденной опции в массив и проверка на двойное определение (на самом деле двойное определение отслживается на этапе парсинга json) */
...
static nxs_cfg_json_state_t nxs_cfg_json_read_val(nxs_process_t *proc, nxs_json_t *json, nxs_cfg_json_par_t *cfg_json_par_el)
{
	...
	if(cfg_json_par_el->type != NXS_CFG_JSON_TYPE_VOID && cfg_json_par_el->proc_function != NULL){
		...
		nxs_log_write_error(proc, "json cfg process error: option have none void type and pointer to process function (option: \"%s\", type: %s)", nxs_string_str(nxs_json_get_key(json)), nxs_json_type_string(json));
	...
	nxs_log_write_error(proc, "json cfg process error: undefined behavior while processing option (option: \"%s\")", nxs_string_str(nxs_json_get_key(json)));
...
static nxs_cfg_json_state_t nxs_cfg_json_type_handler_void(nxs_process_t *proc, nxs_json_t *json, nxs_cfg_json_par_t *cfg_json_par_el)
{
	...
	if(cfg_json_par_el->proc_function == NULL){
		...
		nxs_log_write_error(proc, "json cfg process error: option have void type and no pointer to process function (option: \"%s\")", nxs_string_str(nxs_json_get_key(json)));
...
static nxs_cfg_json_state_t nxs_cfg_json_type_handler_object(nxs_process_t *proc, nxs_json_t *json, nxs_cfg_json_par_t *cfg_json_par_el)
{
	...
	if(nxs_json_type_get(json) != NXS_JSON_TYPE_OBJECT){
		...
		nxs_log_write_error(proc, "json cfg process error: wrong type for value (option: \"%s\", type: %s, expected type: object)", nxs_string_str(nxs_json_get_key(json)), nxs_json_type_string(json));
	...
	if(cfg_json_par_el->sub_els == NULL){
		...
		nxs_log_write_error(proc, "json cfg process error: missing pointer to parameters definition structure for option (option: \"%s\", option type: object)", nxs_string_str(nxs_json_get_key(json)));
...
static nxs_cfg_json_state_t nxs_cfg_json_type_handler_array_object(nxs_process_t *proc, nxs_json_t *json, nxs_cfg_json_par_t *cfg_json_par_el)
{
	...
	if(nxs_json_type_get(json) != NXS_JSON_TYPE_ARRAY){
		...
		nxs_log_write_error(proc, "json cfg process error: wrong type for value (option: \"%s\", type: %s, expected type: array)", nxs_string_str(nxs_json_get_key(json)), nxs_json_type_string(json));
	...
	if(cfg_json_par_el->el_prep_function == NULL){
		...
		nxs_log_write_error(proc, "json cfg process error: missing element's prepare function for array object option type (option: \"%s\")", nxs_string_str(nxs_json_get_key(json)));
	...
	for(i = 0; i < nxs_json_child_get_count(json); i++){
		...
		if(nxs_json_type_get(j) != NXS_JSON_TYPE_OBJECT){
			...
			nxs_log_write_error(proc, "json cfg process error: wrong type for sub-value (option: \"%s\", type: %s, expected type: object)", nxs_string_str(nxs_json_get_key(json)), nxs_json_type_string(j));
		...
		if(cfg_json_par_el->el_prep_function(proc, j, cfg_json_par_el, &cfg_arr) != NXS_CFG_JSON_CONF_OK){
			...
			nxs_log_write_error(proc, "json cfg process error: error in element's prep function while processin array sub-value (option: \"%s\")", nxs_string_str(nxs_json_get_key(json)));
...
static nxs_cfg_json_state_t nxs_cfg_json_type_handler_array_int(nxs_process_t *proc, nxs_json_t *json, nxs_bool_t is_list, nxs_cfg_json_par_t *cfg_json_par_el)
{
	...
	for(i = 0; i < nxs_json_child_get_count(json); i++){
		...
		if(nxs_json_type_get(j) != NXS_JSON_TYPE_INTEGER){
			...
			nxs_log_write_error(proc, "json cfg process error: wrong type for sub-value (option: \"%s\", type: %s, expected type: integer)", nxs_string_str(nxs_json_get_key(json)), nxs_json_type_string(j));
		...
		*p = nxs_json_integer_val(j);
		...
		if(*p < (ssize_t)cfg_json_par_el->min_val){
			...
			nxs_log_write_error(proc, "json cfg process error: value of integer option less then allowed minimum value (option: \"%s\", value: \"%" NXS_JSON_INT_FORMAT "\", allowed values: from '%" NXS_JSON_INT_FORMAT "' till '%" NXS_JSON_INT_FORMAT "')", nxs_string_str(nxs_json_get_key(json)), nxs_json_integer_val(j), (ssize_t)cfg_json_par_el->min_val, (ssize_t)cfg_json_par_el->max_val);
		...
		if(*p > (ssize_t)cfg_json_par_el->max_val){
			...
			nxs_log_write_error(proc, "json cfg process error: value of integer option more then allowed maximum value (option: \"%s\", value: \"%" NXS_JSON_INT_FORMAT "\", allowed values: from '%" NXS_JSON_INT_FORMAT "' till '%" NXS_JSON_INT_FORMAT "')", nxs_string_str(nxs_json_get_key(json)), nxs_json_integer_val(j), (ssize_t)cfg_json_par_el->min_val, (ssize_t)cfg_json_par_el->max_val);
...
static nxs_cfg_json_state_t nxs_cfg_json_type_handler_array_real(nxs_process_t *proc, nxs_json_t *json, nxs_bool_t is_list, nxs_cfg_json_par_t *cfg_json_par_el)
{
	...
	for(i = 0; i < nxs_json_child_get_count(json); i++){
		...
		if(nxs_json_type_get(j) != NXS_JSON_TYPE_REAL){
			...
			nxs_log_write_error(proc, "json cfg process error: wrong type for sub-value (option: \"%s\", type: %s, expected type: real)", nxs_string_str(nxs_json_get_key(json)), nxs_json_type_string(j));
...
static nxs_cfg_json_state_t nxs_cfg_json_type_handler_array_string(nxs_process_t *proc, nxs_json_t *json, nxs_bool_t is_list, nxs_cfg_json_par_t *cfg_json_par_el)
{
	...
	for(i = 0; i < nxs_json_child_get_count(json); i++){
		...
		if(nxs_json_type_get(j) != NXS_JSON_TYPE_STRING){
			...
			nxs_log_write_error(proc, "json cfg process error: wrong type for sub-value (option: \"%s\", type: %s, expected type: string)", nxs_string_str(nxs_json_get_key(json)), nxs_json_type_string(j));
...
static nxs_cfg_json_state_t nxs_cfg_json_type_handler_array_bool(nxs_process_t *proc, nxs_json_t *json, nxs_bool_t is_list, nxs_cfg_json_par_t *cfg_json_par_el)
{
	...
	for(i = 0; i < nxs_json_child_get_count(json); i++){
		...
		if(nxs_json_type_get(j) != NXS_JSON_TYPE_TRUE && nxs_json_type_get(j) != NXS_JSON_TYPE_FALSE){
			...
			nxs_log_write_error(proc, "json cfg process error: wrong type for sub-value (option: \"%s\", type: %s, expected type: true or false)", nxs_string_str(nxs_json_get_key(json)), nxs_json_type_string(j));
...
static nxs_cfg_json_state_t nxs_cfg_json_type_handler_int(nxs_process_t *proc, nxs_json_t *json, nxs_cfg_json_par_t *cfg_json_par_el)
{
	...
	if(nxs_json_type_get(json) != NXS_JSON_TYPE_INTEGER){
		...
		nxs_log_write_error(proc, "json cfg process error: wrong type for value (option: \"%s\", type: %s, expected type: integer)", nxs_string_str(nxs_json_get_key(json)), nxs_json_type_string(json));
	...
	*var = (ssize_t)nxs_json_integer_val(json);
	...
	if(*var < (ssize_t)cfg_json_par_el->min_val){
		...
		nxs_log_write_error(proc, "json cfg process error: value of integer option less then allowed minimum value (option: \"%s\", value: \"%" NXS_JSON_INT_FORMAT "\", allowed values: from '%" NXS_JSON_INT_FORMAT "' till '%" NXS_JSON_INT_FORMAT "')", nxs_string_str(nxs_json_get_key(json)), nxs_json_integer_val(json), (ssize_t)cfg_json_par_el->min_val, (ssize_t)cfg_json_par_el->max_val);
...
static nxs_cfg_json_state_t nxs_cfg_json_type_handler_real(nxs_process_t *proc, nxs_json_t *json, nxs_cfg_json_par_t *cfg_json_par_el)
{
	...
	if(nxs_json_type_get(json) != NXS_JSON_TYPE_REAL){
		...
		nxs_log_write_error(proc, "json cfg process error: wrong type for value (option: \"%s\", type: %s, expected type: real)", nxs_string_str(nxs_json_get_key(json)), nxs_json_type_string(json));
	...
	*var = (double)nxs_json_real_val(json);
	...
	if(*var < cfg_json_par_el->min_val){
		...
		nxs_log_write_error(proc, "json cfg process error: value of real option less then allowed minimum value (option: \"%s\", value: \"%lf\", allowed values: from '%lf' till '%lf')", nxs_string_str(nxs_json_get_key(json)), nxs_json_real_val(json), cfg_json_par_el->min_val, cfg_json_par_el->max_val);
	...
	if(*var > cfg_json_par_el->max_val){
		...
		nxs_log_write_error(proc, "json cfg process error: value of real option more then allowed maximum value (option: \"%s\", value: \"%lf\", allowed values: from '%lf' till '%lf')", nxs_string_str(nxs_json_get_key(json)), nxs_json_real_val(json), cfg_json_par_el->min_val, cfg_json_par_el->max_val);
...
static nxs_cfg_json_state_t nxs_cfg_json_type_handler_string(nxs_process_t *proc, nxs_json_t *json, nxs_cfg_json_par_t *cfg_json_par_el)
{
	...
	if(nxs_json_type_get(json) != NXS_JSON_TYPE_STRING){
		...
		nxs_log_write_error(proc, "json cfg process error: wrong type for value (option: \"%s\", type: %s, expected type: string)", nxs_string_str(nxs_json_get_key(json)), nxs_json_type_string(json));
...
static nxs_cfg_json_state_t nxs_cfg_json_type_handler_bool(nxs_process_t *proc, nxs_json_t *json, nxs_cfg_json_par_t *cfg_json_par_el)
{
	...
	if(nxs_json_type_get(json) != NXS_JSON_TYPE_TRUE && nxs_json_type_get(json) != NXS_JSON_TYPE_FALSE){
		...
		nxs_log_write_error(proc, "json cfg process error: wrong type for value (option: \"%s\", type: %s, expected type: true or false)", nxs_string_str(nxs_json_get_key(json)), nxs_json_type_string(json));
...
static nxs_cfg_json_state_t nxs_cfg_json_par_def_add(nxs_process_t *proc, nxs_array_t *arr, nxs_cfg_json_par_t *cfg_json_par_el)
{
	...
	for(i = 0; i < nxs_array_count(arr); i++){
		...
		if(p->par == cfg_json_par_el){
			...
			if(p->count == 1){
				...
				nxs_log_write_error(proc, "json cfg process error: duplicate entry for option (option: \"%s\")", nxs_string_str(&cfg_json_par_el->name));
	...
	nxs_log_write_error(proc, "json cfg process error: can't find option in def-list (option: \"%s\")", nxs_string_str(&cfg_json_par_el->name));
...
static nxs_cfg_json_state_t nxs_cfg_json_par_def_check(nxs_process_t *proc, nxs_array_t *arr)
{
	...
	for(i = 0; i < nxs_array_count(arr); i++){
		...
		if(p->count == 0 && p->par->required == NXS_YES){
			...
			nxs_log_write_error(proc, "json cfg process error: missing required option (option: \"%s\")", nxs_string_str(&p->par->name));
